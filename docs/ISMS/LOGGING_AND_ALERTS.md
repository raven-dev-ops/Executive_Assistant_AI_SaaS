...
- **Prometheus-style checks via GitHub Actions**: `.github/workflows/p0-alerts.yml` polls the `/metrics/prometheus` endpoint every 15 minutes.
  - Configure repo secret `METRICS_URL` (or `UPTIME_BASE_URL` which will be used as a fallback base URL).
  - If the metrics endpoint is private (for example, Cloud Run with IAM auth), prefer GitHub OIDC/WIF over static bearer secrets:
    - `GCP_WORKLOAD_IDENTITY_PROVIDER` (Workload Identity Provider resource name)
    - `GCP_SERVICE_ACCOUNT` (service account email)
    - The workflow will mint a short-lived Cloud Run ID token and send it as `Authorization: Bearer ...`.
  - Legacy option: set `METRICS_AUTH_HEADER` (e.g., `Authorization: Bearer xxx`).
  - Threshold secrets: `TWILIO_WEBHOOK_THRESHOLD`, `CALENDAR_FAILURE_THRESHOLD`, `AUTH_FAILURE_THRESHOLD`, `OWNER_ALERT_FAILURE_THRESHOLD` (and the newer per-metric thresholds).
  - Optional: `SLACK_WEBHOOK`.
  - Workflow fails (and optionally posts to Slack) when thresholds are exceeded.
