...
| 2025-12-12 | Cloud IAM (prod/staging) | damon.heath | Project `google-mpf-dmxmytcubly9` IAM: removed broad `roles/editor` from `215484517190@cloudservices.gserviceaccount.com`; remaining roles are scoped to build/run/sql/storage. SSO/MFA status not visible from policy (needs org-level toggle). | Removed editor; no additional changes. | `gcloud projects get-iam-policy google-mpf-dmxmytcubly9` (post-removal 2025-12-12) |
