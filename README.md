...
- Stub/demo indicators: when providers are left in stub mode (Twilio/SMS/Stripe/OpenAI), dashboard and API responses remain functional but should be treated as demo. Set the corresponding env vars to move to live mode; guardrails avoid logging secrets and reject invalid keys.
